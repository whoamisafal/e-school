  ...
  _buildMessageComposer() {
    ...
    return Container(
      ...
      child: Row(
        ...
        chat.doc().set(model.toMap()).then((value)async {
         
      
        });
          sendNotification();
           sendNotificationMessage(widget.model.token, message.text, user.displayName+" message you");
 ...
 sendNotification()async{
  try{
        notification.doc().set({
            "title":user.displayName+" message you",
            "body":message.text,
            "timestamp":DateTime.now().millisecondsSinceEpoch,
            "seen":false,
...
@override
  void initState() {
   notification= FirebaseFirestore.instance.collection("Notifications").doc(widget.model.uid).collection("notification");
    if(widget.model.uid.compareTo(user.uid)==1){
if(mounted){
  setState(() {
      chat=FirebaseFirestore.instance.collection("chats").doc(widget.model.uid+user.uid).collection("chat");
  });
}
    }else{
       if(mounted){
         setState(() {
            chat=FirebaseFirestore.instance.collection("chats").doc(user.uid+widget.model.uid).collection("chat");
         });
       }
    }
    try {
